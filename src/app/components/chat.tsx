...
export default function Chat() {
  ...
  const faqs = [
    "Чи є продукція «Біорост» екологічно безпечною?",
    "Які основні види добрив і препаратів пропонує компанія «Біорост»?",
    "Які переваги використання добрив «Біорост» для ґрунту, рослин і людини?",
    "Які контакти компанії «Біорост» для консультацій і замовлення продукції?",
  ];
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 px-2 pb-2 md:bottom-6 md:right-6 md:left-auto md:px-0 z-50">
      {!isOpen && (
        <button
          onClick={toggleChat}
          className="fixed md:relative bottom-6 right-6 flex items-center justify-center w-14 h-14 bg-green-600 rounded-full shadow-lg hover:bg-green-700 transition-colors"
        >
      ...
      {isOpen && (
        <div className="w-full mx-auto max-w-[calc(100%-16px)] md:max-w-none md:w-110 h-[550px] bg-white shadow-xl flex flex-col overflow-hidden rounded-md border border-green-100">
          <div className="bg-green-700 text-white p-3 flex justify-between items-center">
            ...
            <div className="flex gap-x-4">
              ...
              >
                ...
                />
                {hovered && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 px-2 py-1 bg-black text-white text-sm rounded whitespace-nowrap z-10 shadow">
                    Оновити
                  </div>
                )}
              </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-[#E7F5E9]">
            {messages.length === 0 ? (
              <div className="space-y-2">
                <div className="text-center text-green-900 mb-4">
                  Вітаю! Ось популярні запитання:
                </div>
                {faqs.map((faq, index) => (
                  <button
                    key={index}
                    onClick={() => handleFAQClick(faq)}
                    className="w-full text-left p-3 bg-white hover:bg-green-100 rounded-lg transition-colors text-sm text-green-900 border border-green-300"
                  >
                    {faq}
                  </button>
                ))}
                  ...
                  <div
                    ...
                    className={`mb-3 p-3 rounded-lg ${
                      m.role === "user"
                        ...
                        : "bg-white text-green-900 mr-auto border ml-[30px] border-green-300"
                    }`}
                  >
          ...
          <form
            ...
            className="p-3 border-t border-green-200 flex gap-2 bg-white"
          >
            <button
              type="button"
              onClick={() => {
            ...
            <input
              ...
              className="flex-1 p-2 border border-green-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-[#e2fae2] text-green-900"
            />
            ...
            <button
              ...
              className="p-2 text-green-800 hover:text-green-600 disabled:opacity-50 transition-colors"
            >
              <FiSend className="text-lg cursor-pointer" />
            </button>
          </form>
        </div>
      )}
